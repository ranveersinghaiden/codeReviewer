...
export function formatPythonEntryPointImportMatrix(checks) {
    if (checks.length === 0)
        return [];
    const lines = [
...
export function formatCredentialRetrievalMatrix(checks) {
    if (checks.length === 0)
        return [];
    const lines = [
    ...
    for (const check of checks) {
        if (check.unavailableReason) {
            ...
            lines.push(`| \`${check.path}\` | **FINDINGS** — ${check.addedRetrievals} added / ${check.removedRetrievals} removed ` +
