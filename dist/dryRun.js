...
function tail(s, max = MAX_OUTPUT_CHARS) {
    return s.length > max ? `...(truncated)...\n${s.slice(-max)}` : s;
}
async function runStep(step, cmd, args, cwd, timeoutMs = DEFAULT_TIMEOUT_MS) {
 ...
 */
export async function dryRun(repoPath) {
    ...
    if ((await exists(path.join(repoPath, "build.gradle"))) ||
...
async function runNodeStack(repoPath) {
    const steps = [];
    ...
    const installCmd = pnpmLock
        ...
        : yarnLock
            ...
            : pkgLock
                ? ["npm", ["ci"]]
                : ["npm", ["install"]];
    steps.push(await runStep("install", installCmd[0], installCmd[1], repoPath, 8 * 60 * 1000));
...
async function runPythonStack(repoPath) {
    const steps = [];
...
async function hasTool(cwd, tool) {
    return new Promise((resolve) => {
...
async function runMavenStack(repoPath) {
    const steps = [];
...
async function runGradleStack(repoPath) {
    const steps = [];
