 ...
 */
export function analyzeDiff(files: ParsedDiffFile[]): Finding[] {
  const findings: Finding[] = [];
  const rulesMap = rulesForFiles(files.map((f) => f.path));

  for (const file of files) {
    const categories = [...rulesMap.keys()].filter((cat) =>
      rulesForFiles([file.path]).has(cat)
    );
    for (const { line, content } of file.addedLines) {
      for (const check of PATTERN_CHECKS) {
        if (!categories.includes(check.category)) continue;
...
export interface ReviewBundle {
  ...
  findings: Finding[];
  applicableRuleCategories: RuleCategory[];
}

/** Formats the full markdown summary for the review comment / MCP tool response. */
export function formatReviewSummary(bundle: ReviewBundle): string {
...
export function decideReviewEvent(bundle: ReviewBundle): "APPROVE" | "REQUEST_CHANGES" | "COMMENT" {
  ...
  if (dryRunFailed || hasBlocker) return "REQUEST_CHANGES";
  if (bundle.findings.some((f) => f.severity === "warning")) return "COMMENT";
  return "APPROVE";
}
