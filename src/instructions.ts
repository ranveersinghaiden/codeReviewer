...
export interface InstructionDoc {
  source: string; // relative path within the repo
  content: string;
}
 ...
 */
export async function loadInstructions(repoPath: string): Promise<InstructionDoc[]> {
  const docs: InstructionDoc[] = [];

  for (const rel of CANDIDATE_ROOT_FILES) {
    const full = path.join(repoPath, rel);
    const content = await readIfExists(full);
    if (content) docs.push({ source: rel, content });
  }
...
async function readIfExists(fullPath: string): Promise<string | null> {
  try {
...
export function formatInstructionsContext(docs: InstructionDoc[]): string {
  if (docs.length === 0) return "(No coding-agent instruction files found in this repo.)";
  return docs
    .map((d) => `### ${d.source}\n\n${d.content.trim()}`)
    .join("\n\n---\n\n");
}
